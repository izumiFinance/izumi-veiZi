...
describe("test increase unlock time", function () {
    ...
    it("increase amount", async function () {
        ...
        await veiZi.connect(tester).increaseAmount('1', addAmount)

        const deltaGlobalAcc3 = stringDiv(stringMul(stringMul(rewardPerBlock, '10'), q128), stakeiZiAmount);

        const stakeiZiAmount3 = stringAdd(stakeiZiAmount, addAmount);
        ...
        const deltaGlobalAcc4 = stringDiv(stringMul(stringMul(rewardPerBlock, '20'), q128), stakeiZiAmount3);

        const expectReward4 = stringDiv(stringMul(stringAdd(deltaGlobalAcc3, deltaGlobalAcc4), lastVeiZi2), q128);
        ...
        const reward5 = await tryCollect(veiZi, iZi, tester);

        const remainTime5 = String(timestampStart + WEEK * 35 - startTime5);
        const stakingStatus5 = await getStakingStatus(veiZi, '1');

        const lastVeiZi5 = stringMul(slope, remainTime5);
        expect(lastVeiZi5).to.equal(stakingStatus5.lastVeiZi);
        const deltaGlobalAcc5 = stringDiv(stringMul(stringMul(rewardPerBlock, '5'), q128), stakeiZiAmount3);

        const expectReward5 = stringDiv(stringMul(deltaGlobalAcc5, lastVeiZi4), q128);
        expect(reward4).to.equal(expectReward4);
    });
