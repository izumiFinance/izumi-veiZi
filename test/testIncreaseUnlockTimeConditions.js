...
describe("test increase unlock time", function () {
    ...
    it("increase lock of tester, unstaked", async function () {
        const WEEK = Number((await veiZi.WEEK()).toString());
        let ok = true;
        try {
    ...
    it("increase lock of tester, staked", async function () {
        ...
        let ok = true;
        try {
            await veiZi.connect(tester).increaseUnlockTime('1', timestampStart + WEEK * 31);
        } catch(err) {
            // console.log(err);
            ok = false;
        }
    ...
    it("increase lock of other, unstaked", async function () {
    ...
    it("increase lock of other, staked", async function () {
    ...
    it("try to make endtime of lock earlier than before", async function () {
        const WEEK = Number((await veiZi.WEEK()).toString());
        let ok = true;
        try {
            await veiZi.connect(tester).increaseUnlockTime('1', timestampStart + WEEK * 15);
        } catch(err) {
            // console.log(err);
            ok = false;
        }
    ...
    it("increase an expired lock to time before current block.timestamp", async function () {
        const WEEK = Number((await veiZi.WEEK()).toString());
        let ok = true;
        try {

            const startTime = timestampStart + Math.round(WEEK * 25.5);
            await ethers.provider.send('evm_setNextBlockTimestamp', [startTime]);
            await veiZi.connect(tester).increaseUnlockTime('1', timestampStart + WEEK * 25);
        } catch(err) {
            // console.log(err);
            ok = false;
        }
    ...
    it("try to set endtime later than 4 years", async function () {
        const WEEK = Number((await veiZi.WEEK()).toString());
        ...
        try {
            ...
            const startTime = timestampStart + Math.round(WEEK * 25.5);
            await ethers.provider.send('evm_setNextBlockTimestamp', [startTime]);
            await veiZi.connect(tester).increaseUnlockTime('1', timestampStart + Math.round(WEEK * 25.5) + MAXTIME + 1 + WEEK);
        } catch(err) {
            // console.log(err);
            ok = false;
        }
